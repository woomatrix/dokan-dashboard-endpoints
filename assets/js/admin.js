            ...
            this.element.sortable({
                ...
                stop: function( event, ui ) {
                    ...
                    if (($element.hasClass("group")) && ($new_parent.hasClass("group"))) {
                        alert(cddepadmin.group_mixing_text);
                        

                        return false;
                    }

                    if (($element.hasClass("store")) || ($new_parent.hasClass("payment"))) {
                        alert(cddepadmin.setting_moveout);
                    ...
                    if (($element.hasClass("core")) && ($new_parent.hasClass('group')) ) {
                        alert(cddepadmin.core_drop);
                    ...
                    if (($new_parent.hasClass("settings")) ) {
                        alert(cddepadmin.setting_drop);
                    ...
                    if ($new_parent.hasClass('group')) {
                        $element.find('.cddep_parent_field').val($new_parent.attr("keyvalue"));
                    } else {
                        $element.find('.cddep_parent_field').val("none");
                    }
...
$var( function() {
    ...
    $var(".cddep-accordion").accordion( { 
        sortable: true, 
        handle:"h3",
        connectWith:".cddep_group_items"
    ...
    $var(".cddep_group_items").accordion( { 
        sortable: true, 
        handle:"h3",
        connectWith:".cddep-accordion"
       
    });


    $var(".cddep-accordion").find('.cddep_accordion_onoff,.cddep_accordion_remove').on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        setTimeout(function() {
    ...
    $var(".cddep_accordion_remove").on('click',function() {
        var parentkey = $var(this).attr("parentkey");
        if (!confirm(cddepadmin.endpoint_remove_alert)){
          return false;
        }

        if ((parentkey == "store") || (parentkey == "payment")) {
            alert(cddepadmin.settings_removal);
            return false;
        }
        ...
        if (litype == "group") {
            ...
            var coreli = $var(parentitem).find("li.cddep_endpoint.core").length;

            if (coreli == 0) {
                $var(parentitem).fadeOut('slow').remove();
            } else {
                alert(cddepadmin.core_remove_alert);
            }
    ...
    $var('.cddep_accordion_onoff').click(function() {

        var parentkey = $var(this).attr("parentkey");
        
        if ($var(this).is(":checked")) {
            $var(this).parents("li."+ parentkey +"").removeClass('cddep_disabled');
            $var("."+ parentkey +"_hidden_checkbox").val("yes");

        } else {
            
            $var(this).parents("li."+ parentkey +"").addClass('cddep_disabled');
...
$var( function() {
    ...
    $var(".cddep_class_input").tagEditor({
      delimiter: ', ', /* space and comma */
      placeholder: cddepadmin.classplaceholder
    });

    $var("#cddep_reset_tabs_button").on('click',function() {
        var result = confirm(cddepadmin.restorealert);
    ...
    $var('.cddep_show_avatar_checkbox').on("change",function() {
               
        if($var(this).prop("checked")) {
            $var(".cddep_avatar_size_tr").show();
        } else {
            $var(".cddep_avatar_size_tr").hide();
        }
    });
    ...
    $var('.cddep_disabled').on("click",function(event) {
        event.preventDefault();
        alert(cddepadmin.pro_notice);
        jQuery('#cddep_example_modal').modal('hide');
        return false;
    });


    function cddep_sanitize(string) {
        const map = {
          '&': '&amp;',
          '<': '&lt;',
          '>': '&gt;',
    ...
    $var(".cddep_new_end_point").on("click",function(event) {
               
        event.preventDefault();

        var evalue = $var("#cddep_modal_label").val();

        if (evalue == "") {
            
            $var('.cddep_enter_label_alert').html('');
            $var('<p>'+cddepadmin.empty_label_notice+'</p>').appendTo('.cddep_enter_label_alert');
            $var('.cddep_enter_label_alert').show();
            setTimeout(function() {
                $var('.cddep_enter_label_alert').hide();
                $var('.cddep_enter_label_alert').html('');
            }, 2000);
        
        } else {

            var etype = $var('#cddep_hidden_endpoint_type').val();
            var replacetxt = ''+cddepadmin.wait_text+'';
            $var('.cddep_new_end_point').text(replacetxt);
            

            $var.ajax({
                data: {
                    action    : "cddepadmin_add_new_value",
                    row_type  : cddep_sanitize(etype),
                    new_row   : cddep_sanitize(evalue),
                    security  : cddepadmin.nonce,
                    nonce     : $var('#cddep_hidden_endpoint_type').attr("nonce")
                    
                },
                type: 'POST',
                url: cddepadmin.ajax_url,
                success: function( response ) { 
                    window.location.reload();
                }
            });
    ...
    $var('#cddep_example_modal').on('show.bs.modal', function (event) {
        ...
        var modal = $var(this)
        modal.find('.cddep_new_end_point').text("Add New "+ capitalize(etype) +"");
        modal.find('#cddep_hidden_endpoint_type').val(etype);
    ...
    $var(".cddep_icon_source_radio").on("click", function (event) {
    ...
    $var('.cddepvisibleto').on('change',function(){
        $var(this).closest('tr').next('.cddeproles').toggle();
    });


    $var('.cddep_roleselect').select2({
        width:"400px",
